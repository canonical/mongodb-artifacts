...
The `mongos` service runs as the unprivileged `mongodb` user (uid `584792`) and reads the
...
sudo rockcraft.skopeo --insecure-policy copy \
  oci-archive:mongosd_*_amd64.rock \
...
The mongos container automatically generates a keyfile at `/etc/mongod/mongodb-keyfile` (mode `400`,
...
`configsvr` on a Docker network named `mongo-cluster`. We will use the `get-keyfile`
and `set-keyfile` commands to set the keyfile in mongos.
...
- `mongos` reads the keyfile only at startup, so restart the service
...
```bash
docker exec -it mongos mongosh --port 27019
docker exec mongos mongodump --port 27019 --out /tmp/backup
```
