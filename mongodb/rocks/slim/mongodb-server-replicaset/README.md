...
The `mongod` service runs as the unprivileged `mongodb` user (uid `584792`) and reads the
...
```bash
docker exec mongo1 get-keyfile | docker exec -i mongo2 set-keyfile
docker exec mongo1 get-keyfile | docker exec -i mongo2 set-keyfile
...
docker exec rs1 set-keyfile "$key"
```

Or pipe it directly between containers:

```bash
docker exec mongo1 get-keyfile | docker exec -i rs1 set-keyfile
```

Notes:

- `mongod` read the keyfile only at startup, so restart the service
(`docker restart <container>`) after changing the keyfile for it to take effect.
- Every member of a replica set must use the same key. When rotating, propagate
